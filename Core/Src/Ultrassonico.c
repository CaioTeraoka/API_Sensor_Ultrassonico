...
void Aproximacao(void){
	...
	while(1){
		...
		for(int x = 0; x < dist*100000; ++x){} // delay de acordo com a distancia
...
void Alerta_Distancia(uint32_t dist){
	...
	if (dist_atual < dist) HAL_GPIO_WritePin(GPIOA, LED_1_Pin, 1); //Condicional para decisao se acende o led ou não
	else HAL_GPIO_WritePin(GPIOA, LED_1_Pin, 0);
	for(int x = 0; x < 240000; ++x){} //delay de 60ms indicado pelo fabricante do sensor entre uma medição e outra
	}
